...
export default async function Page() {
    async function refreshData() {
        // 'use server'
        revalidatePath('/admin/suketaktifperusahaan')
    }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("suketaktifperusahaan/all", {});
                if (resp.status) {
    ...
    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SuketAktifPerusahaan';
    }

    const deletePost = async (item: any) => {
        try {
            const url = "suketaktifperusahaan/delete/" + item.id;
                ...
                <tbody>
                    {postItem.map((item: any) =>
                        <tr key={item.id} className={"text-white " + (item.status == 0 ? 'bg-slate-500' : 'bg-slate-900')}>
                            ...
                            <td className="p-3">{item.namept}</td>
                            <td className="p-3">
