...
export default async function Page() {
    async function refreshData() {
        // 'use server'
        revalidatePath('/admin/suketpenegasanwil')
    }

    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SuketPenegasanWil';
    }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("suketpenegasanwil/all", {});
                if (resp.status) {
    ...
    const deletePost = async (item: any) => {
        try {
            const url = "suketpenegasanwil/delete/" + item.id;
