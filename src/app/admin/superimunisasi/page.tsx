...
export default async function Page() {
    async function refreshData() {
        // 'use server'
        revalidatePath('/admin/superimunisasi')
    }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("superimunisasi/all", {});
                if (resp.status) {
    ...
    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SuperImunisasi';
    }

    const deletePost = async (item: any) => {
        try {
            const url = "superimunisasi/delete/" + item.id;
