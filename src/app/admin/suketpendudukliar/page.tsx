...
export default async function Page() {
    async function refreshData() {
        // 'use server'
        revalidatePath('/admin/suketpendudukliar')
    }

    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SuketPendudukLiar';
    }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("suketpendudukliar/all", {});
                if (resp.status) {
    ...
    const deletePost = async (item: any) => {
        try {
            const url = "suketpendudukliar/delete/" + item.id;
