...
export default async function Page() {
    ...
    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SuketKekerabatan';
    }
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("suketkekerabatan/all", {});
                if (resp.status) {
    ...
    const deletePost = async (item: any) => {
        try {
            const url = "suketkekerabatan/delete/" + item.id;
