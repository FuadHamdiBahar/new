...
export default async function Page() {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const resp = await getAPI("sptodesa/all", {});
                if (resp.status) {
    ...
    function getDownloadURL(filename: string): string {
        return getBaseUrl() + 'download?filename=' + filename + '&form=SpToDesa';
    }

    const deletePost = async (item: any) => {
        try {
            const url = "sptodesa/delete/" + item.id;
